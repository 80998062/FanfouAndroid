 ...
 * <p>
 * It tracks a {@link RequestState} and an {@code error} for each {@link RequestType}.
 * <p>
...
public class PagingRequestHelper {
    ...
    /**
     * Adds a new listener that will be notified when any request changes {@link RequestState state}.
     *
    ...
    public boolean runIfNotRunning(@NonNull RequestType type, @NonNull Request request) {
        ...
        synchronized (mLock) {
            ...
            queue.mStatus = RequestState.RUNNING;
            queue.mFailed = null;
    ...
    @GuardedBy("mLock")
    private RequestState getStatusForLocked(RequestType type) {
    ...
    void recordResult(@NonNull RequestWrapper wrapper, @Nullable Throwable throwable) {
        ...
        synchronized (mLock) {
            ...
            if (success) {
                queue.mFailed = null;
                queue.mStatus = RequestState.SUCCESS;
            } else {
                queue.mFailed = wrapper;
                queue.mStatus = RequestState.FAILED;
            }
            if (hasListeners) report = prepareStatusReportLocked();
    ...
    public static final class StatusReport {
        ...
        public boolean hasRunning() {
            return initial == RequestState.RUNNING || before == RequestState.RUNNING || after == RequestState.RUNNING;
        }
        ...
        public boolean hasError() {
            return initial == RequestState.FAILED || before == RequestState.FAILED || after == RequestState.FAILED;
        }
    ...
    class RequestQueue {
        ...
        @NonNull
        RequestState mStatus = RequestState.SUCCESS;
