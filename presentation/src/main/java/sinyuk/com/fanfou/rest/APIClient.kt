...
import sinyuk.com.common.utils.UrlEscapeUtils
...
fun initFanfouClient(application: Context,
...
fun initTwitterClient(application: Context, cached: Boolean = false): OkHttpClient {

    val timeout: Long = 30
    val max = (1024 * 1024 * 10).toLong() // 10 MiB
    val logging = HttpLoggingInterceptor(HttpLoggingInterceptor.Logger {
        TimberDelegate.tag("TwitterAPI").d(it)
    })
...
fun generateCredential(token: String?, secret: String?, request: Request,
    ...
    val timestamp = mockTimestamp ?: FanfouAccessTokenTask.Clock().millis()
